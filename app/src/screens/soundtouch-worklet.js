...
class PitchProcessor extends AudioWorkletProcessor{
  constructor(){
    ...
    this._st.tempo=1.0;
    this._iL=new Float32Array(512*2);
    this._iO=new Float32Array(512*2);
    this.port.onmessage=e=>{
      if(e.data?.ratio!=null){this._ratio=e.data.ratio;this._st.pitch=e.data.ratio;}
      if(e.data?.reset){this._st.clear();this._st.pitch=this._ratio;}
    };
  }
  process(inputs,outputs){
    const i0=inputs[0]?.[0],i1=inputs[0]?.[1]||inputs[0]?.[0];
    const o0=outputs[0]?.[0],o1=outputs[0]?.[1];
    if(!i0||!o0)return true;
    const n=i0.length;
    for(let i=0;i<n;i++){this._iL[i*2]=i0[i];this._iL[i*2+1]=i1?i1[i]:i0[i];}
    this._st.inputBuffer.putSamples(this._iL,0,n);
    this._st.process();
    const avail=this._st.outputBuffer.frameCount;
    const rd=Math.min(avail,n);
    if(rd>0){
      this._st.outputBuffer.receiveSamples(this._iO,rd);
      for(let i=0;i<rd;i++){o0[i]=this._iO[i*2];if(o1)o1[i]=this._iO[i*2+1];}
    }
    if(rd<n){o0.fill(0,rd);if(o1)o1.fill(0,rd);}
    return true;
  }
}
