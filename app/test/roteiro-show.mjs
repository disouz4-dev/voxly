...
async function lerEstado() {
  return g(`(async () => {
    ...
    return {
      ...
      ordemRegra: VoxlyOrdem.ordenarFila(banco.filter(d => ['aguardando','tocando','confirmando','pronto'].includes(d.status) && d.slot !== 'espera'), { ultimoFoiCafe: _ultimoFoiCafe }).map(d => d.id),
    };
  })()`);
}
